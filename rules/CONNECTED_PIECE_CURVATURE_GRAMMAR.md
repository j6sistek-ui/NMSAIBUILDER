...
## Curve-Follow Transform Propagation Contract
*(absorbed from CURVE_FOLLOW_TRANSFORM_PROPAGATION_PROTOCOL.md, v4.00.00 — restored as an enforceable contract)*

**Mandatory** whenever a build repeats a part along a curve, spiral, ring, path, outline, rib, wing edge, rail, or decorative sweep (spires, conical roofs, domes, crowns, rings). Treat this as **transform propagation, not independent stamping** — the failure mode behind invented spire rib-cages.

A curve-follow build must declare:
...
Required behavior: position is sampled along the curve; facing follows the curve tangent (or declared tangent projection); up/right are derived from the curve frame; roll/bank is continuous unless a hard corner is intentional; endpoints and gaps are intentional and declared.

Failure conditions (treat as orientation defects): curve pieces use inconsistent/random rotations; endpoint gaps undeclared; samples not tied to tangent/normal frames; a curve-follow object presented as validated contact without step/contact proof.
