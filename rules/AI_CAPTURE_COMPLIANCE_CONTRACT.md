...
## Hand-off visibility (delivered scripts must be recoverable)

A delivered build script must leave **every collection and object visible and included in the view layer** at hand-off. The user receives a build they can see and edit, not a staged review state.

- Do **not** bake `hide_viewport = True`, view-layer `exclude = True`, or `collection.hide_viewport(True)` into the delivered build script. These flags **resist `Alt+H`** — the user cannot recover the parts in Blender without hunting through the Outliner.
- Review-time isolation (hiding shell/front/decor to expose interior, per-collection captures) is the **review addon's** job and must use **recoverable `hide_set()`** with a restore pass — never the un-recoverable flags above.
- Semantic review collections (STRUCTURAL_SHELL, FRONT_FOCAL_FACE, ACCESS_AND_INTERIOR, ROOF_SPIRES, DECOR_DETAIL_REVIEW, SCENE_CONTEXT) organize parts for review; they must all ship **visible + included**. Isolation is applied at capture time and reverted, not delivered.
- `run_gate.py` enforces this in the static linter (`no baked-hidden collections/parts`): a delivered script containing `hide_viewport=True` / view-layer `exclude=True` **FAILS** the machine-checkable gate.
