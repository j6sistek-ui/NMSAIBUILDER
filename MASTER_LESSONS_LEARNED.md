...
## Large cultural/landmark build methodology (consolidated from retired 01_MASTER_REFERENCE)
For large nostalgic/cultural builds (Taj Mahal, Hogwarts, Wayne Manor class): macro silhouette first → subsystem studies → screenshot validation → protected baselines → late-stage part optimization. See `MASTER_BUILD_RECIPES_AND_PLACEMENT_GUIDANCE.md` for the recipe wiring and `rules/FOCAL_BUILD_GEOMETRY_STRATEGY_PROTOCOL.md` for focal geometry.
