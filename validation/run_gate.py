#!/usr/bin/env python3
"""
run_gate.py (v5.07.00) — executable validation gate.
...
def main():
    ...
    if libpath:
        try:
            ...
            for d in data:
                ...
                lib_ext[oid] = tuple((v if v is not None else 0.0) for v in vals)
        except Exception as e:
            libdiff = f"LIBRARY_READ_ERROR: {e}"

    conn_active = bool(libpath) and bool(lib_ext) and len(placed) >= 2
    ...
    print("SCRIPT VALIDATION — EXECUTABLE GATE (v5.07.00)")
    ...
    if not conn_active:
        reason = "no library" if not libpath else ("<2 placed parts" if len(placed) < 2 else "no extents in library")
    ...
    if not _sheet or not _os.path.exists(_sheet) or len(placed) < 1:
        ...
        if _nv or _nomap:
            print("  note: status is informational; non-validated parts are allowed. A part becomes validated via a")
            print("    map-only proof build plus user sign-off (PART_PLACEMENT_MAP_SCHEMA).")
    print("--- gate verdict ---")
