# 00 — KICKOFF / INTAKE GATE (5.07.00) — REQUEST ROUTER + FEATURE RECIPE ROUTER FIRST — REQUEST ROUTER + EXECUTION KERNEL FIRST — REQUEST ROUTER FIRST, EVERY TIME
...
2. On a new chat, complete the `ONBOARDING_VALIDATION_BUILD` before the first user build — a real, intentionally minimal floor/wall/roof micro-build run through the full process (real `run_gate`, gate-verified `BUILD COMPLIANCE MANIFEST`), per `ONBOARDING_VALIDATION_BUILD.md`. Treat it exactly like a user build.
