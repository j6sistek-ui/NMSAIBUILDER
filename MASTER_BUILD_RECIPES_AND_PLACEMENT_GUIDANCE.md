...
Status: **canonical recipe-system method** — the single source of truth for how recipes are STORED and UTILIZED. All recipe storage and utilization wiring flows through the layers named here; do not introduce a parallel recipe store.

## The single recipe method (storage + utilization wiring)

**Storage (one store of record):**
- `toolkit/PLACEMENT_RECIPE_LIBRARY.json` — the authoritative recipe definitions (the only hand-maintained recipe store). `toolkit/PLACEMENT_RECIPE_LIBRARY.md` is a generated human-readable pointer into this JSON, never an independent copy.
- `toolkit/validated_recipes/*.py` — importable reference implementations for solved part-family logic (e.g. stairs, C_TRIFLOOR); a script mirrors/calls these and records it in `USED_PART_LOGIC`.
- `data/RUNTIME_BEHAVIORS.json` — build-level assembly recipe behaviors consumed by the resolver (Wave 3 runtime spine).

**Lookup (routing a request to a recipe):**
- `data/FEATURE_RECIPE_ROUTE_INDEX.json` — feature-intent → recipe (the machine-readable form of the Priority order below).
- `data/JSON_RECIPE_SIGNATURE_INDEX.json` — current-build JSON signature → recipe.

**Utilization (one wiring path):** `validation/part_context_resolver.py` walks the Priority order — feature route, then JSON signature / validated recipe, then part-specific rule, then part-map component validation — and emits recipe candidates + assembly behaviors into the build packet; `validation/run_gate.py` enforces it via `feature_recipe_lookup_check.py`. No recipe is consumed outside this path.
